...
## Correcciones descubiertas durante la ejecución (Task 5)

`IdempotencyE2ETest` reveló un segundo bug de infraestructura, también de Fase 0, invisible hasta ahora porque nadie externo a la red de Docker había intentado producir directamente a Kafka: **el broker solo anunciaba su listener interno (`KAFKA_ADVERTISED_LISTENERS: PLAINTEXT://kafka:9092`)**. Un cliente Kafka que se conecta desde el host (`localhost:9092`, puerto mapeado) completa el *bootstrap* correctamente, pero al pedir metadata el broker le informa que debe reconectarse a `kafka:9092` — un hostname que solo resuelve dentro de la red de Docker. El productor de test entraba en un loop de reintentos silencioso hasta expirar (`TimeoutException: Expiring 1 record(s) ... 122238 ms has passed`).

**Fix:** se agregó un segundo listener (`EXTERNAL`) al servicio `kafka` en `docker-compose.yml`, expuesto en el puerto `9094` y anunciado como `localhost:9094` — sin tocar el listener `PLAINTEXT` interno (`kafka:9092`) que usan los servicios de negocio entre sí. `IdempotencyE2ETest` usa `localhost:9094` como `bootstrap.servers`. Verificado recreando el contenedor de Kafka con la config nueva, reejecutando `kafka-topics-init`, y confirmando que el test pasa en ~5 segundos (antes fallaba tras ~2 minutos de reintentos).

## Correcciones descubiertas durante la ejecución (Task 6)

La primera versión de `ResilienceE2ETest` seguía literalmente el diseño original del plan (detener `account-service` **antes** de llamar a `POST /transfer`), y falló con `500 Internal Server Error` — no `202` como se esperaba. Causa real: la validación síncrona de `transfer-service` (`AccountClient.fetchAccount`, ver Fase 2) necesita `account-service` arriba para poder validar existencia/ownership/estado/saldo **antes** de iniciar la saga. Detener el servicio antes de la request rompe esa validación previa, no el procesamiento asíncrono — el escenario "detener antes de transferir" simplemente no es equivalente a "detener a mitad de la saga" en esta arquitectura, algo que el self-review original no había verificado contra el comportamiento real del código de Fase 2.

**Fix:** se invirtió el orden — la transferencia se envía primero (con `account-service` arriba, la validación síncrona pasa y la saga arranca), y **luego** se detiene el contenedor, con una ventana corta (`Thread.sleep(1000)`) antes de reiniciarlo. Esto es, en sí, una carrera contra el consumidor Kafka de `account-service` (puede que ya haya procesado el evento antes de que el `docker stop` surta efecto) — se documenta explícitamente en el propio test que el objetivo no es garantizar que se interrumpa a mitad de proceso, sino verificar que el sistema tolera la caída/reinicio del contenedor sin perder el evento ni corromper el estado, sea cual sea el momento exacto en que ocurre. Verificado: el test pasa consistentemente y el saldo converge correctamente tras el reinicio.
